    ...
    async def __call__(self, handler, event: types.Update, data: dict):
        ...
        async with aiosqlite.connect(DB_NAME) as db:
            ...
            if event.message and event.message.text and not event.message.text.startswith("/"):
                ...
                async with db.execute("SELECT clicks FROM users WHERE user_id = ?", (user_id,)) as cur: clicks = (await cur.fetchone())[0]
...
async def cmd_start(message: types.Message):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute("SELECT balance FROM users WHERE user_id = ?", (user_id,)) as cur: exists = await cur.fetchone()
        if not exists:
            async with db.execute("SELECT welcome_bonus, ref_bonus FROM settings WHERE id = 1") as cur: bonuses = await cur.fetchone()
...
async def verify_join(call: types.CallbackQuery):
    if await check_membership(call.from_user.id):
        async with aiosqlite.connect(DB_NAME) as db:
            async with db.execute("SELECT welcome_bonus FROM settings WHERE id = 1") as cur: bonus = (await cur.fetchone())[0]
...
async def send_appeal(message: types.Message, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute("SELECT file_id FROM submissions WHERE task_id = ? AND worker_id = ?", (task_id, worker_id)) as cur: sub = await cur.fetchone()
    ...
    if sub:
        ...
        for admin in SUPER_ADMINS:
            try: await bot.send_photo(admin, photo=sub[0], caption=f"🚨 <b>NEW APPEAL</b>\nTask ID: {task_id}\nWorker: {worker_id}\n\n💬 <b>Worker's Defense:</b> {reason}", reply_markup=kb, parse_mode="HTML")
            except: pass
...
async def resolve_appeal(call: types.CallbackQuery):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute("SELECT reward, category FROM tasks WHERE task_id = ?", (task_id,)) as cur: task = await cur.fetchone()
        async with db.execute("SELECT status FROM submissions WHERE task_id = ? AND worker_id = ?", (task_id, worker_id)) as cur: sub = await cur.fetchone()
...
async def admin_panel(message: types.Message):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute("SELECT maint_mode FROM settings WHERE id = 1") as cur: maint = (await cur.fetchone())[0]
...
async def admin_stats(call: types.CallbackQuery):
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute("SELECT COUNT(*), SUM(balance) FROM users") as cur: users_data = await cur.fetchone()
        async with db.execute("SELECT COUNT(*), SUM(completed_slots) FROM tasks") as cur: tasks_data = await cur.fetchone()
...
async def admin_back(call: types.CallbackQuery):
    await call.message.delete(); await admin_panel(call.message)
...
async def admin_process_input(message: types.Message, state: FSMContext):
    data = await state.get_data(); action, text = data['action'], message.text
    async with aiosqlite.connect(DB_NAME) as db:
        try:
            if action == "set_welcome": await db.execute("UPDATE settings SET welcome_bonus = ? WHERE id = 1", (float(text),))
            elif action == "set_ref": await db.execute("UPDATE settings SET ref_bonus = ? WHERE id = 1", (float(text),))
            elif action == "set_min": await db.execute("UPDATE settings SET min_sc = ? WHERE id = 1", (float(text),))
            elif action == "set_adsense_link": await db.execute("UPDATE settings SET adsense_link = ? WHERE id = 1", (text,))
            elif action == "set_adsense_rew": await db.execute("UPDATE settings SET adsense_reward = ? WHERE id = 1", (float(text),))
            elif action == "set_monetag_link": await db.execute("UPDATE settings SET monetag_link = ? WHERE id = 1", (text,))
            elif action == "set_monetag_rew": await db.execute("UPDATE settings SET monetag_reward = ? WHERE id = 1", (float(text),))
            elif action == "set_monetag_freq": await db.execute("UPDATE settings SET ad_freq = ? WHERE id = 1", (int(text),))
            elif action == "gift":
                target, amount = text.split(); target = target.replace("@", "")
                ...
                target = text.replace("@", ""); async with db.execute("SELECT is_banned FROM users WHERE user_id = ? OR username = ?", (target, target)) as cur: current = (await cur.fetchone())[0]
                await db.execute("UPDATE users SET is_banned = ? WHERE user_id = ? OR username = ?", (1 if current == 0 else 0, target, target))
            elif action == "broadcast": await db.execute("INSERT INTO notifications (user_id, message) SELECT user_id, ? FROM users", (f"📢 BROADCAST:\n{text}",))
            await db.commit(); await message.answer("✅ <b>Update Successful!</b>", parse_mode="HTML")
        except: await message.answer(f"❌ Error. Try again.")
    await state.clear()
...
async def fetch_task_list(call: types.CallbackQuery):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        async with db.execute(query, (category, user_id, user_id, user_id)) as cur: tasks = await cur.fetchall()
...
async def view_task_btn(call: types.CallbackQuery):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute("SELECT target, conditions, reward FROM tasks WHERE task_id = ?", (task_id,)) as cur: task = await cur.fetchone()
...
async def verify_task(call: types.CallbackQuery):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute("SELECT target, reward FROM tasks WHERE task_id = ?", (task_id,)) as cur: task = await cur.fetchone()
    try:
    ...
    except: await call.answer("❌ Broken task. Admin removed bot.", show_alert=True)
...
async def handle_screenshot(message: types.Message, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        async with db.execute("SELECT creator_id, reward FROM tasks WHERE task_id = ?", (task_id,)) as cur: creator = await cur.fetchone()
        await db.commit()
    ...
    if creator:
        ...
        try: await bot.send_photo(creator_id, photo=file_id, caption=f"🔔 <b>New Proof ({category})</b>\n💰 <b>Reward:</b> {reward} SC\nReview:", reply_markup=kb, parse_mode="HTML")
        except: pass
...
async def promote_target(message: types.Message, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute("SELECT min_sc FROM settings WHERE id = 1") as cur: base_min = (await cur.fetchone())[0]
        async with db.execute("SELECT balance FROM users WHERE user_id = ?", (message.from_user.id,)) as cur: bal = (await cur.fetchone())[0]

    if data['category'] == "Telegram":
        if bal < base_min:
            await state.clear(); return await message.answer(f"❌ <b>Low Balance!</b>\nMin cost is {base_min} SC.", parse_mode="HTML")
...
async def promote_reward(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(f"✅ <b>Price Accepted!</b>\nAt {rew} SC per person, you can afford max <b>{max_slots} people</b>.\nHow many do you want?", parse_mode="HTML")
...
async def promote_slots(message: types.Message, state: FSMContext):
    try:
        ...
        async with aiosqlite.connect(DB_NAME) as db:
            ...
            await db.execute("INSERT INTO tasks (creator_id, category, target, conditions, reward, total_slots) VALUES (?, ?, ?, ?, ?, ?)", (user_id, data['category'], data['target'], data['conditions'], data['reward'], slots))
            await db.commit()
...
async def btn_profile(message: types.Message):
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute("SELECT balance, referrals FROM users WHERE user_id = ?", (message.from_user.id,)) as cur: user = await cur.fetchone()
...
async def buy_coins(call: types.CallbackQuery, state: FSMContext):
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute("SELECT rate_usdt, rate_ngn FROM settings WHERE id = 1") as cur: rates = await cur.fetchone()
...
async def process_receipt(message: types.Message, state: FSMContext):
    ...
    for admin in SUPER_ADMINS:
        try: await bot.send_photo(admin, photo=file_id, caption=f"🚨 <b>NEW RECEIPT</b>\nFrom: {user_id}", reply_markup=kb, parse_mode="HTML")
        except: pass
    await state.clear()
...
async def admin_deposit_handler(call: types.CallbackQuery):
    if call.from_user.id not in SUPER_ADMINS: return
    parts = call.data.split("_"); action, user_id = parts[1], parts[2]
    if action == "app":
...
async def view_notifications(message: types.Message):
    user_id = message.from_user.id
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute("SELECT notif_id, message FROM notifications WHERE user_id = ? AND is_read = 0", (user_id,)) as cur: notifs = await cur.fetchall()
...
async def view_adsense(message: types.Message):
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute("SELECT adsense_link, adsense_reward FROM settings WHERE id = 1") as cur: settings = await cur.fetchone()
...
async def btn_referral(message: types.Message):
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute("SELECT ref_bonus FROM settings WHERE id = 1") as cur: ref_bonus = (await cur.fetchone())[0]
...
async def main():
    await init_db()
    keep_alive()  
    print("✅ V15 ULTIMATE EDITION ONLINE: Support & Appeals Active!")
